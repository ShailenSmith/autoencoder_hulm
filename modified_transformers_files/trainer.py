...
### sneaky environment variables
TEST = os.getenv("TESTING","False").lower() in ('true', '1', 't') #################################
DEBUG = TEST ###
GTPL = os.getenv("GTPL","False").lower() in ('true', '1', 't') #################################
custom_mask = os.getenv("CUSTOM_MASK","False").lower() in ('true', '1', 't') #################################
if TEST:
...
class Trainer:
    ...
    def predict(
        self, test_dataset: Dataset, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "test", return_only_inputs_labels=False ###
    ) -> PredictionOutput:
        """
        ...
        eval_loop = self.prediction_loop if self.args.use_legacy_prediction_loop else self.evaluation_loop
        if custom_mask and GTPL: ###
            output, masked_inputs = eval_loop(
                test_dataloader, description="Prediction", ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix,
                return_only_inputs_labels=True, custom_mask_ds=test_dataset ###
            ) ###
            return (output.predictions, output.label_ids, masked_inputs) ### this output.predictions is actually the target probs because of GTPL

        elif return_only_inputs_labels or GTPL: ###
            output, masked_inputs = eval_loop(
                test_dataloader, description="Prediction", ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix, return_only_inputs_labels=True ###
            ) ###
            return (output.predictions, output.label_ids, masked_inputs) ### this output.predictions is actually the target probs because of GTPL
            if DEBUG:
                # pdb.set_trace()
    ...
    def evaluation_loop(
        ...
        ignore_keys: Optional[List[str]] = None,
        metric_key_prefix: str = "eval",
        return_only_inputs_labels = False, ###
        custom_mask_ds = None, ###
    ) -> EvalLoopOutput:
        """
        ...
        if custom_mask_ds is not None: ###
            custom_masked_input_ids = custom_mask_ds['masked_input_ids'] ### not padded
            custom_labels = custom_mask_ds['kept_labels'] ### not padded
        
        # Main evaluation loop
        for step, inputs in enumerate(dataloader):
            if custom_mask_ds is not None: ### replace each iteration with our custom mask and labels
                if DEBUG:
                    pdb.set_trace()
                    pass
                inputs['input_ids'][0] = torch.Tensor(custom_masked_input_ids[step]) ###
                inputs['labels'][0] = torch.Tensor(custom_labels[step]) ###
                if DEBUG:
                    pdb.set_trace()
                    pass
            ...
            if GTPL: ### calculate target probabilities to just return those and the labels
                with torch.no_grad():
                    B, T, C = logits.shape
                    logits = logits.view(-1, C)
                    assert logits.max().item() < 88.6, f"step: {step}, logitmax: {logits.max()}" ### overflow exp value
                    # neg_logits = logits - logits.max()
                    cts = logits.exp()
                    cts_sum = cts.sum(1, keepdims=True)
                    probs = cts / cts_sum
                    target_probs = probs[torch.arange(T), labels.view(-1)] ### (T)
                    if DEBUG:
                        # pdb.set_trace()
                        pass
                    ### this 'logits' is actually the target probs, but we try to smuggle them through the rest of the processing like this
                    logits = torch.Tensor([[[prob.item()] for prob in target_probs]]) ### [1, T, 1] because logits are [1, T, C]
        ...
        if GTPL: ### the all_preds here should actually be only the target probabilities, already with softmax and mean applied
            if DEBUG:
                # pdb.set_trace()
                pass
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        loss, outputs = self.compute_loss(model, inputs, return_outputs=True) ### no change but important line. This loss is just one number
