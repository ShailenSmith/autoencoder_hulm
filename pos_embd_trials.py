...
def run_trials(data_path, model_path, run_name, pos_embd_strat="load_512"):
    ...
    def objective(trial: optuna.Trial, args: TrainingArguments):
        ...
        args.learning_rate=trial.suggest_float("learning_rate", low=3e-5, high=3e-3, log=True)
        ...
        trainer = Trainer(
            ...
            eval_dataset=ds['validation'], #.select(np.arange(3)),
    ...
    study.optimize(func=lambda trial: objective(trial, training_args), n_trials=50, callbacks=[wandbc])  
...
os.environ["WANDB_PROJECT"] = "pos_embd_trials"

data_paths = [sample_chunked_path, sample_chunked_path, sample_chunked_path]
