...
def run_trials(data_path, model_path, run_name, pos_embd_strat="load_512"):
    ...
    def objective(trial: optuna.Trial, args):
        ...
        config_dict = {
            ...
            'attention_probs_dropout_prob' : trial.suggest_float("attention_probs_dropout_prob",
                                                    low=0.01, high=0.5, log=True),
        ...
        args.learning_rate=trial.suggest_float("learning_rate", low=5e-5, high=5e-2, log=True)
        args.weight_decay=trial.suggest_float("weight_decay", low=0.001, high=0.01, log=True)
    ...
    wandb_kwargs = {"project": "huggingface"}
    wandbc = WeightsAndBiasesCallback(wandb_kwargs=wandb_kwargs, as_multirun=True)
    study.optimize(func=lambda trial: objective(trial, training_args), n_trials=5, callbacks=[wandbc])   
...
if True:
    ...
    run_names = ["non_dsep_trials", "dsep_trials"]
